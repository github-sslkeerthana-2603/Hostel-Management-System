...
<div class="content">
  <a id='chk20' style="background-color:black; font-size: 16px;color:white;" href="RegistrationRecChiefWarden.php">Record of previous registrations</a>
   <a id='chk21' style="background-color:black; font-size: 16px;color:white;" href="RegistrationStatus.php">View status1</a>
...
<div class="content">
  ...
  <div class="dropdown-content">
    <a href="ProfileChiefWarden.php">View Profile</a>
    ...
    <div style="text-align: center;width:80%; height: 80%; justify-content: center; margin-left:5%;">
       <h2>PREVIOUS RECORDS OF OUTINGS</h2>
...
<table id="myTable">
  <tr class="header">
    ...
    <th style="width:14%;">Local/non-local</th>
  </tr>
 ...
 <label style="color:white;"for="room_no">Room no</label><br>
  ...
  <input type="checkbox" id="local_nonlocal" name="local_nonlocal" value="local_nonlocal" onclick="myFunction1();">
 <label style="color:white;"for="local_nonlocal">Local/non-local</label><br>
...
switch($usertype){
    ...
    $query1="select status from outingapplications";
	break;
	case "WARDEN":
	$query="select * from outing where hostel_name='munneru'";
    $query1="select status from outingapplications where status='pending' ";
	break;
	case "CARETAKER":
	$query="select * from outing where hostel_name='Bhima'";
    $query1="select status from outingapplications where status='' ";
	break;
	default:
	echo "error";
}
$query_result=mysqli_query($con, $query) or die( mysqli_error($con));
$query1_result=mysqli_query($con, $query1) or die(mysql_error($con));
...
?>
<script>
var roll = <?php echo json_encode($arr1); ?>;
var name1 = <?php echo json_encode($arr2); ?>;
var app=<?php echo json_encode($arr); ?>;
var hostel_name = <?php echo json_encode($arr3); ?>;
var room_no = <?php echo json_encode($arr4); ?>;
var status1 = <?php echo json_encode($arr5); ?>;
var date=<?php echo json_encode($arr7); ?>;
var loc =<?php echo json_encode($arr6); ?>;
alert(status1);
    for (var i=0;i<roll.length;i++){
        /*if (i===0){
            status1.push("approved");
        }else if (i===2){
            status1.push("pending");
        }else{
            status1.push("denied");
        }*/
	
        document.getElementById('myTable').innerHTML+="<tr class='row'><td>"+app[i]+"</td><td>"+date[i]+"</td><td>"+roll[i]+"</td>\n\
<td>"+name1[i]+"</td><td>"+hostel_name[i]+"</td><td>"+room_no[i]+"</td><td>"+loc[i]+"</td></tr><br>";
...
for (var i=0;i<status1.length;i++){
    if (status1[i]==='pending'){
        pending_count++;
    }else if (status1[i]==='approved'){
        approved_count++;
    }else{
        rejected_count++;
    }
...
document.addEventListener('DOMContentLoaded', function MakeVisible() {
